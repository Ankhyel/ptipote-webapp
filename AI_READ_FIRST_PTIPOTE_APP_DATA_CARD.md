...
### Progression et Renouvellement P’TIBUG
...
- Le premier Trait utilise les paliers I à III des niveaux 1 à 3. Le niveau rend seulement un palier éligible : l’action volontaire dans la Nurserie vérifie toujours Pattern, maîtrise, données, matériaux et Énergie. Un P’TIBUG doit être affecté à la Nurserie pour appliquer ou améliorer un Trait.
- Le Renouvellement exige niveau 3, premier Trait III, présence dans la Nurserie et stock matériel/Cellules vide. Il conserve identifiant, espèce, nom, niveau, XP, Modules, stock et premier Trait ; il définit `isRenewed`, `renewedAt`, `renewalCount`, sans créer de duplicat. Les éventuels matériaux, Bio-batteries et Énergie sont réglés dans `progression` (zéro par défaut prototype).
- Après Renouvellement, le second Trait est distinct du premier et suit les paliers I/II/III des niveaux 4/5/6. Les champs de sauvegarde sont `biologicalTraitId`/`biologicalTraitLevel` pour le premier Trait et `secondTraitId`/`secondTraitLevel` pour le second. La migration initialise les nouveaux champs sans modifier les P’TIBUG existants et supprime seulement un doublon exact de Trait II invalide.
...
- Le Dashboard P’TIBUG édite `progression` (niveaux, XP, rendement, énergie et Renouvellement) et `moduleCapacity` (capacités et tous les coûts), publiés sous `gameConfigs/zone0.zone0Settings.ptibug`. `ptibug_config.dart` demeure le fallback versionné hors ligne; les sauvegardes joueur ne sont jamais écrites dans la configuration distante.
...
Le Marché utilise des piles de vente indépendantes : deux piles identiques ne sont jamais fusionnées et la logique commune de prélèvement consomme d’abord la pile la plus entamée. Les demandes habitantes sont temporaires (10–20 minutes par défaut), sans pénalité à l’expiration. Elles sont créées indépendamment du stock : par défaut, un tiers de la population formule une demande par heure et l’intervalle moyen est modulé aléatoirement de ±10 à ±30 %. Une demande choisit le nom d’un habitant actif du Cœur lorsqu’il existe. Il n’existe plus de cycle de vente autonome : tout prélèvement du stock répond obligatoirement à une demande habitant, manuellement ou par le Distributeur après son délai d’une minute ; les contrats du Sourcier restent séparés.
...
Les emplacements de magasins spécialisés sont pilotés par `market.specializedShopSlotsByMarketLevel` : V1 affiche 3 emplacements au Marché niveau 2, puis 5 au niveau 3 et 7 au niveau 4. Le premier magasin spécialisé existant est désormais proposé dès le niveau 2 ; les emplacements ultérieurs restent réservés aux constructions spécialisées à compléter par leur règle de coût dédiée.
