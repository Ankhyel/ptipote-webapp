...
- Les missions de Lisière créent maintenant des `PTibugDataCell` fermées selon la durée, l'intensité et la table du biome P'TIBUG associée aux anciens biomes de mission. Une Cellule contient toujours cinq entrées, avec une chance configurable de Cellule neutre; l'ouverture depuis l'onglet Progression du Kernel crédite définitivement la réserve de données et crée un message Kernel.
...
- Les Traits biologiques sont permanents : un P'TIBUG n'en reçoit qu'un. Son nom affiché devient par exemple `Hyme Pollinisateur II`. Les huit Traits V1 disposent de coûts par niveau, modifiables depuis le Dashboard ; les effets systémiques (Cellules, Toxine, énergie et sécurité locale) sont appliqués dans les services de jeu, sans créer de ressource matérielle fictive.
...
- Les Capsules migrées reçoivent des identifiants d'origine, créateur et certification de compatibilité. Les Cellules historiques sont complétées à cinq entrées lors du chargement, sans modifier les données déjà ouvertes. L'éditeur Dashboard des grandes tables (familles par biome, Patterns, Traits, Modules et Capsules) reste à enrichir au-delà des champs scalaires V1.
...
- Les effets systémiques sont appliqués dans `ptipote-app/lib/features/game/zone0_game_state.dart` : Éclaireur augmente la chance de Cellule en Lisière, Filtreur augmente le poids Toxine, Économe réduit le coût d'Énergie de fabrication des Modules et Stabilisateur augmente la sécurité locale du biome lié à chaque cycle de production. Ces bonus ne créent pas de fausse ressource dans l'inventaire.
